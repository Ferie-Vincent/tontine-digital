...
<div
    ...
    x-show="show"
    x-transition
    {{ $attributes->merge(['class' => "rounded-lg border p-4 {$config['bg']} {$config['border']} {$config['text']}"]) }}
>
    <div class="flex items-start gap-3">
        ...
        @if($dismissible)
        <button @click="show = false" class="flex-shrink-0 p-1 -m-1 rounded opacity-70 hover:opacity-100 transition">
