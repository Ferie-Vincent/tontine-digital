<x-layouts.app :title="'Historique - ' . $tontine->name">
    <x-slot:header>
        <div class="flex items-center gap-3">
            <a href="{{ route('tontines.show', $tontine) }}" class="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors">
                <svg class="w-5 h-5 text-slate-500 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            </a>
            ...
            <div>
                <h2 class="text-xl font-bold text-slate-800 dark:text-white">Historique d'activite</h2>
    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4">
            <form method="GET" action="{{ route('tontines.activity', $tontine) }}" class="flex flex-col sm:flex-row gap-3">
                <div class="flex-1">
                    <select name="action" onchange="this.form.submit()" class="w-full rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white text-sm focus:ring-blue-500 focus:border-blue-500">
                        <option value="all">Toutes les actions ({{ $activities->total() }})</option>
                ...
                @if(request('action') && request('action') !== 'all')
                    <a href="{{ route('tontines.activity', $tontine) }}" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                        Effacer le filtre
                    </a>
                @endif
                @if(auth()->user()->canManage($tontine))
                    <a href="{{ route('tontines.activity.export', $tontine) }}" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg transition-colors">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
            @if($activities->isEmpty())
                <div class="text-center py-12">
                    ...
                    <p class="text-slate-500 dark:text-slate-400">L'historique d'activité de cette tontine est vide pour le moment.</p>
                </div>
            @else
